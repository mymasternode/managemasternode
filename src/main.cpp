...
#if defined(NDEBUG)
#error "ManageMyNode cannot be compiled without assertions."
#endif
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ManageMyNode
...
{
    RenameThread("managemynode-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMANAGEMYNODEMinted()
{
...
void RecalculateZMANAGEMYNODESpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
...
bool RecalculateMANAGEMYNODESupply(int nHeightStart)
{
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MANAGEMYNODE to zCDM
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ManageMyNode: We use certain sporks during IBD, so check to see if they are
