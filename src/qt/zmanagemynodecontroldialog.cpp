...
#include "ui_zmanagemynodecontroldialog.h"
...
std::list<std::string> ZManageMyNodeControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZManageMyNodeControlDialog::listMints;

ZManageMyNodeControlDialog::ZManageMyNodeControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZManageMyNodeControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZManageMyNodeControlDialog::~ZManageMyNodeControlDialog()
{
    delete ui;
}

void ZManageMyNodeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZManageMyNodeControlDialog::updateList()
{
...
void ZManageMyNodeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZManageMyNodeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZManageMyNode_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZManageMyNodeControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZManageMyNodeControlDialog::GetSelectedMints()
{
...
void ZManageMyNodeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
