...
#include "zmanagemynodecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMMN ought to be enough for anybody." - Bill Gates, 2017
    ui->zMMNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMMNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMMN->setEnabled(false);
        ui->pushButtonMintzMMN->setToolTip(tr("zMMN is currently disabled due to maintenance."));

        ui->pushButtonSpendzMMN->setEnabled(false);
        ui->pushButtonSpendzMMN->setToolTip(tr("zMMN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMMNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMMN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMMN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMMN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMMN in ") +
...
void PrivacyDialog::on_pushButtonSpendzMMN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMMN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMMN
        sendzMMN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMMN
    sendzMMN();
}

void PrivacyDialog::on_pushButtonZManageMyNodeControl_clicked()
{
    ZManageMyNodeControlDialog* zMMNControl = new ZManageMyNodeControlDialog(this);
    zMMNControl->setModel(walletModel);
    zMMNControl->exec();
}

void PrivacyDialog::setZManageMyNodeControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMMNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMMN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ManageMyNode Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMMNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMMN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MANAGEMYNODE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMMN</b>";
    ...
    // use mints from zMMN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZManageMyNodeControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZManageMyNodeControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmanagemynode selector in case it was used
    ZManageMyNodeControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMMN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ManageMyNode, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMMN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMMN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMMN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMMN "));
    ui->labelzMMNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMMNSyncStatus->setVisible(fShow);
}
